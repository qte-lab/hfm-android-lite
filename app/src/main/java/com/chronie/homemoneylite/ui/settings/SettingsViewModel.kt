...
class SettingsViewModel @Inject constructor(
    ...
    // 服务器端被动同步进度（被搜索方）
    val serverSyncProgress: StateFlow<com.chronie.homemoneylite.domain.sync.SyncProgressInfo> =
        syncManager.getDeviceSyncManager().syncProgress

    // 收到的同步请求（被搜索方）
    private val _incomingSyncRequest = MutableStateFlow<com.chronie.homemoneylite.domain.sync.SyncRequestInfo?>(null)
    val incomingSyncRequest: StateFlow<com.chronie.homemoneylite.domain.sync.SyncRequestInfo?> = _incomingSyncRequest.asStateFlow()
     ...
     * 同步请求回调实现
     */
    override suspend fun onSyncRequest(requestInfo: com.chronie.homemoneylite.domain.sync.SyncRequestInfo): Boolean {
